#ifndef MARLIN_TM_ABCI
#define MARLIN_TM_ABCI
...
public:
	...
	Abci(std::string datadir) : connect_timer(this), path(datadir + "/tm.ipc") {
		pipe.delegate = this;
		connect_timer_cb();

		if(boost::filesystem::exists("./.marlin/keys/abci/tm")) {
			// Load existing keypair
			std::ifstream sk("./.marlin/keys/abci/tm", std::ios::binary);
			...
			std::ofstream sk("./.marlin/keys/abci/tm", std::ios::binary);
